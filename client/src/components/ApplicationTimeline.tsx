...
export default function ApplicationTimeline({ 
  ...
  const timeline: TimelineItem[] = [
    ...
    {
      status: status === "rejected" ? "Application Rejected" : "Certificate Approved",
          ...
          {timeline.map((item, index) => (
            <div key={index} className="flex gap-4" data-testid={`timeline-item-${index}`}>
              <div className="flex flex-col items-center">
                {item.completed ? (
                  <CheckCircle2 className={`h-6 w-6 ${item.current ? "text-primary" : "text-muted-foreground"}`} />
                ) : item.current ? (
                ...
                {index < timeline.length - 1 && (
                  <div className={`w-0.5 h-12 mt-2 ${item.completed ? "bg-primary" : "bg-border"}`} />
                )}
              </div>
              <div className="flex-1 pb-8">
                <h4 className={`font-semibold ${item.current ? "text-foreground" : "text-muted-foreground"}`}>
                  {item.status}
                </h4>
